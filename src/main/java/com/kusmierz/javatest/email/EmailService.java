...
public class EmailService {
    ...
    // injecting the EmailRepository for data access
    @Autowired
    ...
    // This method creates an Email and sets the state to DRAFT -> it should be sent separately
    ...
    // This method receives the ID of the email that should be changed and also gets an updated version,
    // which delivers the new information like EmailFrom, EmailTo, EMailCC, EmailSubject and EmailBody.
    // The ID and createdDate won't change, as LastUpdateDate will be changed automatically
    public Email updateEmail(Long id, Email updatedEmail) {
        ...
        if(draftEmail != null && draftEmail.getState() == EmailState.DRAFT) {
            ...
            draftEmail.setEmailCC(updatedEmail.getEmailCC());
    ...
    // Because emails get the state DRAFT after creation, this method allows to send a draftemail and
    // and change the state to SENT
